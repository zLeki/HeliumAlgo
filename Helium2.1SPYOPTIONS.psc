//@version=5
strategy("Helium 🎈 V 6.4.2 SPY", overlay=true, precision=0, explicit_plot_zorder=true, max_labels_count=500)
...
sensitivty_custom = input.float(0.00, "Custom sensitivity value",step=0.01, minval=0, maxval=50)
...
if (isMarketOpen)
    if (bull and not na(buy) and not alertTriggered and canTradeToday and limitOrders)
        if reverse
            ...
            strategy.entry("BUY CALLS @"+str.tostring(close), strategy.long)
    ...
    if (bear and not na(sell) and not alertTriggered and canTradeToday and limitOrders)
        if reverse
            ...
            strategy.entry("BUY PUTS @"+str.tostring(close), strategy.short)
